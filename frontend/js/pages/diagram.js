...
async function pageDiagram() {
  ...
  html += '<div class="page-header"><h2>📊 数据库模型图</h2><div class="page-desc">自动布局 ｜ 滚轮缩放 ｜ 拖拽平移 ｜ 点击表进入详情</div></div>';
  ...
  html += '</div>';
  html += '</div></div>';
  setTimeout(function() { loadSchemaOptions(); ensureCytoscape(); }, 0);
  return html;
}
...
async function loadDiagram() {
  ...
  vp.innerHTML = '<div style="text-align:center;padding:60px;color:var(--text-secondary)">加载中...</div>';
  ...
  try {
    ...
    vp.innerHTML = '';
    renderCytoscape(vp, tables, edges, modelDetails);
  } catch(e) {
    vp.innerHTML = '<div class="empty-state"><p>加载失败: ' + e.message + '</p></div>';
  }
}

function buildNodeLabel(t, detail) {
  var cols = detail ? (detail.columns || []) : [];
  var lines = [t.table_name];
  lines.push('─'.repeat(Math.max(22, t.table_name.length + 6)));
  var maxCols = 15;
  for (var i = 0; i < Math.min(cols.length, maxCols); i++) {
    var c = cols[i], prefix = c.is_primary_key ? '🔑' : '  ';
    var typeStr = c.logical_type + (c.type_length ? '(' + c.type_length + (c.type_scale ? ',' + c.type_scale : '') + ')' : '');
    var colLine = prefix + ' ' + c.column_name;
    while (colLine.length < 24) colLine += ' ';
    colLine += ' ' + typeStr;
    lines.push(colLine);
  }
  if (cols.length > maxCols) lines.push('  ... +' + (cols.length - maxCols) + ' more');
  return lines.join('\n');
}

function renderCytoscape(vp, tables, edges, modelDetails) {
  if (!window.cytoscape) { vp.innerHTML = '<div class="empty-state"><p>加载可视化库中，请稍后刷新重试</p></div>'; return; }

  var elements = [];
  tables.forEach(function(t) {
    elements.push({ data: { id: 't' + t.id, label: buildNodeLabel(t, modelDetails[t.id]), tableId: t.id, tableName: t.table_name }, classes: 'table-node' });
  });
  ...
  cy.on('tap', 'node', function(ev) {
    ...
    if (diagramState.mode === 'fk') {
      if (!diagramState.fkSource) {
        ...
        completeFKUI(node.data('tableId'));
      }
  ...
  diagramState.cy = cy;
  setTimeout(function() { if (cy) cy.fit(undefined, 50); }, 400);
}

function completeFKUI(targetModelId) {
  ...
  var input = prompt('请输入外键关系: 源列名,目标列名\n示例: user_id,id');
  if (!input) return;
  var parts = input.split(',').map(function(s) { return s.trim(); });
  if (parts.length < 2) { toast('请输入源列名和目标列名', 'error'); return; }
  api.post('/models/' + src.modelId + '/foreign-keys', { fk_name: 'fk_' + parts[0], column_name: parts[0], ref_model_id: targetModelId, ref_column_name: parts[1] }).then(function() { toast('外键已创建', 'success'); loadDiagram(); }).catch(function(e) { toast(e.message, 'error'); });
}

function fitDiagram() { if (diagramState.cy) diagramState.cy.fit(undefined, 50); }
function resetDiagram() { if (diagramState.cy) { diagramState.cy.zoom(1); diagramState.cy.center(); } }

function startFK(modelId, colName) { if (diagramState.cy) diagramState.cy.nodes().forEach(function(n) { if (n.data('tableId') === modelId) { n.emit('tap'); return false; } }); }
...
function editColumnOnDiagram(modelId, colId) {
  ...
  api.get('/models/' + modelId).then(function(r) {
    var cols = r.data.columns || [], col = cols.find(function(c) { return c.id === colId; });
    if (!col) { toast('字段未找到', 'error'); return; }
    col.column_name = parts[0]; if (parts[1]) col.logical_type = parts[1].toUpperCase(); if (parts[2]) col.comment = parts[2];
    api.put('/models/' + modelId + '/columns', { columns: cols }).then(function() { toast('字段已更新', 'success'); loadDiagram(); }).catch(function(e) { toast(e.message, 'error'); });
  });
}
